...
class EmptyColumnsCleaner:
    ...
    def __remove_empty_columns(self, df):
        ...
        df = df.applymap(lambda x: np.nan if isinstance(x, str) and len(str(x)) > 0 and x.isspace() else x)
        df = df.applymap(lambda x: np.nan if re.match(r'^\\+$', str(x)) else x)
        ...
        columns_to_remove = [col for col in df.columns if self.__non_empty_values_ratio(df, col) >= threshold and df[col].dtype == 'object']
        df = df.drop(columns=columns_to_remove)

        return df

    def __remove_whitespace_and_newlines(self, s):
        return re.sub(r'^\s+|\s+$', '', str(s), flags=re.MULTILINE)
